...
describe('ReturnRequest_EE_06', function () {
    let AWB;
    let OrderID;
    beforeEach(function () {
...
//To mark an order delivered API 
    ...
    it('ReturnRequests_05_05',function(){
       ...
       misc.sidebarMenuItems().each(($el, index, $list) => {
        ...
        filters.getInputBoxForSearchFilter().type(OrderID)   //this.data.newRequestsOrderID
                 ...
                 filters.OrderDetailsLineItem().then(($value) => {
                     ...
                     if (length === 1) {
                         filters.OrderIDFromtheOrderdetailsLineItem().then(($el)=>{
                             ...
                             if(OrderIDForReturn.includes(OrderID)){     //this.data.newRequestsOrderID
