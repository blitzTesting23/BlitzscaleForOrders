...
//Verify the settlement method filter dropdown is clickable and all the options are present and the checkbox is clickable for the pickup failed tab 
it('ReturnRequests_07_01', function () {
   ...
   misc.sidebarMenuItems().each(($el, index, $list) => {
    ...
    filters.filterPlaceHolder().find('p').each(($el, index, $list) => {
        ...
        if (FilterOption.includes('Settlemment Method')) {
            cy.wait(1000)
            $el.click();
            cy.wait(1000)
        }
                 ...
                 if (length === 1) {
                    cy.log('Only One order Item is present for the applied filter option ')
                     filters.OrderIDFromtheOrderdetailsLineItem().then(($el)=>{
                         ...
                         filters.settlementMethodForPickupFailed().should('have.text',this.data.settlementMethod_option)
                         cy.log(OrderID)
                     })
                     ...
                     if (length > 1) {
                        ...
                        filters.settlementMethodForPickupFailed().each(($el, index, $list) => {
...
//Verify the Auto settlement filter dropdown is clickable and all the options are present and the checkbox is clickable in Pickup failed tab 
it('ReturnRequests_07_02', function () {
    const Nudge=new NudgesAndColumns();
    const filters=new Filters();
    const misc=new SidebarAndMisc();
    cy.visit(Cypress.env('url'))
...
//Verify the Shipping partner filter dropdown is clickable and all the options are present and the checkbox is clickable in the pickup failed tab
it('ReturnRequests_07_03', function () {
    const Nudge=new NudgesAndColumns();
    const filters=new Filters();
    const misc=new SidebarAndMisc();
    cy.visit(Cypress.env('url'))
...
//Verify on clicking on More details option in Order details all the necessory field values are present  in Pickup faield requests tab
it('ReturnRequests_07_04', function () {
    const Nudge=new NudgesAndColumns();
    const filters=new Filters();
    const misc=new SidebarAndMisc();
    cy.visit(Cypress.env('url'))
   ...
   misc.sidebarMenuItems().each(($el, index, $list) => {
    ...
    filters.getInputBoxForSearchFilter().type(this.data.OrderIdForPickupFailed)
             ...
             filters.OrderDetailsLineItem().then(($value) => {
                 ...
                 if (length === 1) {
                     filters.OrderIDFromtheOrderdetailsLineItem().eq(0).then(($el)=>{
                         ...
                         cy.log('The order ID searched ',this.data.OrderIdForPickupFailed)
                         if(OrderID===this.data.OrderIdForPickupFailed)
                         {
                         ...
                         filters.spanforMoredetails().eq(6).should('have.text','Pickup Failed')
...
//Verify search filter by order ID is working and  is displaying the valid filtered results  for Pickup failed  requests tab 
it('ReturnRequests_07_15', function () {
    const Nudge=new NudgesAndColumns();
    const filters=new Filters();
    const misc=new SidebarAndMisc();
    cy.visit(Cypress.env('url'))
             ...
             filters.OrderDetailsLineItem().then(($value) => {
                 ...
                 if (length === 1) {
                     filters.OrderIDFromtheOrderdetailsLineItem().then(($el)=>{
                         ...
                         cy.log('The Order ID present on the dashboard',OrderID)
                         cy.log('The Order ID searched ',this.data.OrderIdForPickupFailed)
                        if(OrderID.includes(this.data.OrderIdForPickupFailed)){
                            cy.log('The searched ID is present in the table')
                         }
                     })
...
//Verify search filter by AWB no is working and  is displaying the valid filtered results  for Pickup failed  requests tab 
it('ReturnRequests_07_16', function () {
    const Nudge=new NudgesAndColumns();
    const filters=new Filters();
    const misc=new SidebarAndMisc();
    cy.visit(Cypress.env('url'))
   ...
   misc.sidebarMenuItems().each(($el, index, $list) => {
    ...
    filters.getInputBoxForSearchFilter().type(this.data.AWB_NumberForPickupFailed)
    filters.getSearchSVG().click()    
             ...
             filters.OrderDetailsLineItem().then(($value) => {
                 ...
                 if (length === 1) {
                     filters.awbNumberfield().then(($el)=>{
                         ...
                         cy.log('The AWB searched is',this.data.AWB_NumberForPickupFailed)
                        if(awb.includes(this.data.AWB_NumberForPickupFailed)){
...
//Verify search filter by customer number is working is displaying the valid filtered results for Pickup failed  requests tab 
it('ReturnRequests_07_17', function () {
    const Nudge=new NudgesAndColumns();
    const filters=new Filters();
    const misc=new SidebarAndMisc();
    cy.visit(Cypress.env('url'))
