...
describe('ReturnRequest_EE_01', function () {
    ...
    //approve request from  from new requests tab with Book Return Shipment: Yes,QC : Yes ,Auto Settlement :Yes and check for entries in expense ledger 
       ...
       misc.sidebarMenuItems().each(($el, index, $list) => {
        ...
        filters.searchResultCard().then((cardforresults)=>{   
           ...
           misc.sidebarMenuItems().each(($el, index, $list) => {
            ...
            cy.wait(1000)
            misc.expenseLedgeramount().click()
            cy.wait(1000)
            filters.getSearchFilterDropDown().click({force: true})    
            cy.wait(1000)  
